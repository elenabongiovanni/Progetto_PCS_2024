...
void findIntersections(const unsigned int &id, FractureMesh &mesh){
    ...
    for(unsigned int i=id+1; i<mesh.NumFractures; i++){
        ...
        if(planeF[0]==planeFConf[0] && planeF[1]==planeFConf[1] && planeF[2]==planeFConf[2] && dF == dFConf){ //condizone di complanarità
            ...
            if(intersectionsF[0][0] <= intersectionsFC[0][0]){
                ...
                if(intersectionsFC[1][0]<intersectionsF[0][0]){
                    ...
                    double z1FC = (-dFConf - planeFConf[0]*intersectionsF[0][0] - planeFConf[1]*intersectionsF[0][1])/planeFConf[2]; //quota z1FC della fracture FC
                    double z2F = (-dF - planeF[0]*intersectionsFC[1][0] - planeF[1]*intersectionsFC[1][1])/planeF[2]; //quota z2F della fracture F
        ...
        if(inter){
            ...
            // id della traccia composto da id delle due fratture
            Vector2i TrId;
            TrId ={id,i};
            //cout << TrId<<endl;
            //Trace.traceId.push_back(TrId); dà errore
            // lunghezza della traccia
            double lunghezzatraccia = dist(trace[0],trace[1]);
            newTrace.length = lunghezzatraccia;

            mesh.vecTrace.push_back(newTrace);

            //cout << distanzainF<< "   " << lunghezzatraccia << "   "<<tol<< endl;
            if((abs(lunghezzatraccia-distanzainF))<tol){
                cout << "figura " << id << " e' passante  "<< endl;
                f.listPas.push_back(newTrace);
                f.Tips.push_back(false);
            }else{
                f.listNonpas.push_back(newTrace);
                cout << "figura " << id << " non e' passante"<<endl;
                f.Tips.push_back(true);
            }
            mesh.MapTrace[id] = newTrace;
            //cout << distanzainFC<< "   " << lunghezzatraccia << "   "<<tol<< endl;
            if((abs(lunghezzatraccia-distanzainFC))<tol){
                ...
                cout << "figura " << i << " non e' passante"<<endl;
                f.Tips.push_back(true);
            }
            mesh.MapTrace[i] = newTrace;
        }

        }

          /*  //ordino le liste
            f.listPas.length.sort(greater<double>());
            cout << "Ordinato in modo decrescente: ";
            for (double val : f.listPas.length) {
                cout << val << " ";
            }
            cout << endl;


            f.listNonpas.length.sort(greater<double>());
            cout << "Ordinato in modo decrescente: ";
            for (double val : f.listNonpas.length) {
                cout << val << " ";
            }
            cout << endl;
